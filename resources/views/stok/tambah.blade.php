                ...
                <div class="col-md-7">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control nama-barang" readonly
                            data-harga="{{ $stok->barang->harga_beli }}"
                            value="({{ $stok->barang->barang_kode }}) {{ $stok->barang->barang_nama }}">
                        <small class="error-nama-barang form-text text-danger"></small>
                    </div>
                </div>
                <div class="col-md-7">
                    <div class="form-group">
                        <label>Supplier:</label>
                        <input type="text" class="form-control nama-supplier" readonly
                            value="({{ $stok->supplier->supplier_kode }}) {{ $stok->supplier->supplier_nama }}">
                        <small class="error-nama-supplier form-text text-danger"></small>
                    </div>
                </div>
...
<script>
    $(document).ready(function () {
        ...
        $('#form-container').on('change keyup', '.jumlah-input', function () {
            updateItem();
        });

        function updateItem() { 
            $('.item-row').each(function () {
                const barang = $(this).find('.nama-barang');
                const harga = parseFloat(barang.data('harga')) || 0; // Ambil harga dari data attribute
